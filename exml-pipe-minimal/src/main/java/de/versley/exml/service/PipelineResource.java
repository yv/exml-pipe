...
public class PipelineResource {
    static private GlobalConfig conf;
    ...
    Optional<Pipeline<TuebaDocument>> getPipeline(String name) {
        ...
        if (annotators.isPresent()) {
            ...
            for (Annotator anno: annotators.get()) {
                pipeline.addStage(anno);
            }
            synchronized(conf) {
                System.err.println("PipelineResource: loading models for "+name);
                pipeline.loadModels();
                System.err.println("PipelineResource: done for "+name);
            }
    ...
    public TuebaDocument pipeText(String body, @PathParam("name") String name)
    {
        ...
        ExmlDocBuilder builder = new ExmlDocBuilder(parts[0]);
